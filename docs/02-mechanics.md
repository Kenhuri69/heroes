...
> **obtient le Graal** (`PlayerState.hasGrail`, événement `GrailFound`) — **un seul
> Graal par carte** : déterré par un joueur, il n'est plus fouillable par les
> autres (revue 2026-09b M5) — un
...
> **État livré (profil économique par joueur, lot L5)** : `PlayerState.economyBonus
> { incomePercent?, growthPercent? }` — pourcentages **opaques** appliqués au revenu
> quotidien (bâtiments à effet `income` **et** mines, projection du HUD comprise) et
> à la croissance hebdomadaire (villes **et** habitations de carte, via le helper
> partagé `weeklyGrowthOf`). Posé par les données (`PlayerSetup`) : le client y
> projette son cran de difficulté pour les sièges **IA** — le moteur, lui, ne
> connaît aucun cran. Champ **optionnel** ⇒ absent = facteur 1, **pas de bump
...
- **Moral** (−3..+3) : proba d'un **tour bonus** (moral positif : 4 %/point) ou d'un **tour sauté** (négatif : 4 %/point, symétrique). Le moral positif ne se tire qu'après une action **réelle** (déplacement, attaque, tir, sort) : **ni Attendre ni Défendre** ne le déclenchent (fidélité HoMM — revue 2026-09b M9 : le tour bonus annulait la défense choisie). Armée multi-factions : −1 moral par faction supplémentaire (les morts-vivants ne subissent/ne donnent pas de moral).
...
> **État livré** : le catalogue réellement interprété par le moteur — `data/core/abilities.json` — compte **35 capacités** : `flying`, `shooter`, `noRetaliation`, `doubleAttack`, `undead`, `mark`, `consumeMarks`, `demonform`, `symbiosis`, `shieldWall`, `unlimitedRetaliation`, `charge`, `magicResistance` (autonome, plus seulement porté par `demonform`), `lifeDrain` (lot A2a), `incorporeal`, `strikeAndReturn` (lot A2b), `curseOnHit` (lot A2c), `aura`, `moraleImmune` (lot A3a), `swarm` (lot A3b), `areaAttack` (lot A3c), `devourMarks` (lot A2d), `breathAttack` (lot A3d), `taunt` (lot A2e), `poisonSting` (lot A2f), `firstStrike` (lot A2g), et `spellcaster` (lot A2h) — plus `performer`, `banishable`, `rebirth`, `spellImmune` (CAP-SPELLIMMUNE : immunité aux sorts hostiles), `healPerRound`, `replenishAmmo` et `barrier` livrés depuis. *Revue 2026-08 (lot L9)* : **toutes** sont interprétées ET portées par au moins une unité de faction. `resurrectAlly`, jadis annoncée « pas encore interprétée », **n'existe plus** comme capacité : l'effet est rendu par le `spellcaster` générique (Ange, CAP-LIFE.1) — l'entrée de locale correspondante a été retirée.
...
Sémantique des capacités du catalogue (valeurs de départ ; **35** au 2026-08) :
...
| `spellcaster(spellId, charges, power)` | A2h : la pile lance le sort embarqué `spellId` (catalogue partagé) jusqu'à `charges` fois/combat, Pouvoir effectif `power` (dégâts/soin/durée) ; cibles comme un sort de héros (ennemi damage/debuff, allié heal/buff), sans riposte. Piloté par l'IA/auto-combat **et** jouable à la main (bouton de lancer de la pile active, lot CAP-CAST UI) — Prêtresse (soin ×2) |
