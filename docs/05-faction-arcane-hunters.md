...
> capacité `spellcaster` du Bibliothécaire **livrée (CAP-CAST données)** : câblée
> `spellcaster(entraves-runiques, ×2, power 3)` — le choix **Entraves** (−3 vitesse,
> universellement utile) plutôt que Silence évite un tour d'IA gâché sur une pile
> non-lanceuse ; jouable par l'IA/auto ET à la main (bouton « Sort (unité) »,
> CAP-CAST UI). `faction:sim` inchangé (0 blowout) ; **`poisonSting` livrée (A2f)**
...
- Chaque charge : la cible subit **+5 % de dégâts** des unités Arcane Hunters et des sorts de Traque.
...
| 6 | **Chasseresse de l'Abîme** | 55 | 15 | 11 | 11–17 | 9 | 2 | 1280 or + 2 mercure | `shooter(8, noMeleePenalty)`, `mark`, `pinningShot` (consomme 2 Marques : cible immobilisée 1 round, les volants tombent) |
| 7 | **Manticore de Dressage** | 145 | 18 | 18 | 24–36 | 11 | 1* | 2600 or + 2 mercure + 1 gemme | `flying`, `noRetaliation`, `mark`, `poisonSting` |
...
Bâtiments communs : cf. doc 02 §4.1 (skins « académie »). **Graal**
(`arcane-hunters-grail`, gaté par le méta-puzzle obélisques → fouille) : source
d'Essence — `factionResourceIncome` Essence +12/j. Cf. doc 02 §2.2 (table
...
|--------|-------------|----------------------|------------------------|------------------|
| **Maître de Chasse** | Might | 35/20/20/25 | Chasse rituelle N, Tir N | Les Marques donnent +7 % dégâts au lieu de +5 % |
