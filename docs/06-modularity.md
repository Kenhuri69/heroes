...
6. **Équilibrage** : le test `balance.test.ts` (paquet `@heroes/content`, lancé par `pnpm test`) rejoue des combats auto à valeur d'or égale et vérifie qu'aucune faction ne domine ; le simulateur CLI dédié **existe** — `pnpm faction:sim` (`packages/tools/src/faction-sim.ts`, Alpha 4.17). Il produit **trois lectures** (plan `faction-sim-fidelity`) : (a) **duel valeur-égale** (winrate d'un combat unique — canari de stats brutes, a trouvé l'outlier Dragon d'Ombre, seul déclencheur du code de sortie non-nul ; *revue 2026-09* : joué sur un **terrain neutre par paire** — le premier terrain franchissable natif d'aucun des deux camps — le `grass` fixe d'avant offrait +1 vitesse/+1 moral aux factions natives de la plaine dans tous leurs duels) ; (b) **matrice d'attrition** — chaque camp (armée budget + **héros de faction**) enchaîne des vagues fraîches croissantes en **reportant survivants + relève**, ce qui **valorise la nécromancie & le sustain** qu'un duel unique ne peut voir (`simulateHeroCombat`, moteur) ; (c) **gauntlet de survie** — vagues survécues contre une rotation escaladante des autres factions (même yardstick pour toutes ⇒ robustesse comparable). *Limitation assumée* : la résonance/essence (gagnée en combat mais dépensée hors combat) reste hors périmètre — elle ne sous-estime que Vox, déjà la plus forte au duel. Enseignement : le duel seul est trompeur (Necropolis y paraît la plus faible alors que la nécromancie la remonte au 2ᵉ/3ᵉ rang au gauntlet).
