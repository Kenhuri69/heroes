...
- 🚧 **Faction suivante — Shadow Court (elfes noirs / Dungeon HoMM)** : cadrage livré (`docs/17-faction-shadow-court.md`, plan `.claude/plans/phase-17-faction-shadow-court.md`) — pendant sombre de la Sylvan Court, signature **Magie Irrésistible** (1 point d'extension générique `irresistibleMagic` : la magie destructrice du héros ignore la résistance + bonus plat borné), lineup 7 tiers, ressources `sulfur`/`gems`, héros canon Raelag & Shadya. Décomposition 17.1 cadrage ✅ → 17.2 données → 17.3 signature → 17.4 équilibrage/finitions. Prochain **test de modularité** du pipeline data-driven.
