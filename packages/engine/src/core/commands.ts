...
export interface PlayerSetup {
  ...
  /**
   * Héros nommé du roster (H-NAMED, doc 02 §1.2) — id opaque résolu via
...
export type Command =
  | {
      ...
      /**
       * Roster de héros nommés résolu par le contenu (H-NAMED, doc 02 §1.2), indexé
       * par `heroId` → identité déclarative. Sert à résoudre l'identité du héros à la
       * création (`PlayerSetup.startingHeroId`) ; non stocké dans l'état (comme
       * `houseCatalog`/`specialtyCatalog` — l'identité résolue vit sur le héros).
       */
