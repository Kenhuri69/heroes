#!/usr/bin/env python3
"""
extract_siege_ensemble.py — Découpe du TABLEAU ensembliste peint (itération 8).

Entrée : la peinture générée sur `siege-ensemble-template.png` (même cadrage).
Le masque `siege-ensemble-mask.png` isole la fortification (peinte EN CONTEXTE
⇒ toutes les jonctions — mur↔porte, porte↔chaussée, mur↔tours, états — sont
dans l'art). Sorties :

  assets/combat/siege-run.png            — le RUN complet (tours + courtine +
                                           porte + seuil), 2 px/bp, RGBA
  assets/combat/siege-run-band-<état>.png— bandes-étalons d'1 rangée (intact/
                                           cracked/razed) découpées aux rangées
                                           étalons du tableau
  assets/combat/siege-piece-arrow-tower.png — tour de tir (clé existante)
...
def main() -> None:
    ...
    mask = Image.open(MASK).convert("L").filter(ImageFilter.GaussianBlur(2))
    rgba = painted.convert("RGBA")
    rgba.putalpha(mask)

    def crop_bp(box: tuple[float, float, float, float]) -> Image.Image:
        ...
        return part.resize((w_out, h_out), Image.LANCZOS)
    ...
    arrow = crop_bp(tuple(cuts["arrow"]))
    ...
    preview = Image.new("RGB", (run.width + arrow.width + 3 * 20, max(run.height, arrow.height) + 40), (96, 116, 74))
    ...
    (COMBAT / "siege-kit-source.json").write_text('{"source": "tableau ensembliste Gemini (combat-siege-kit.md v3)"}\n')
